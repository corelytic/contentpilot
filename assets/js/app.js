(function () {
  ...
  // Core configuration used across the local workspace lifecycle.

  const STORAGE_KEY = "contentpilot.workspace.v1";
  const MAX_UNDO = 25;
  const STAGES = ["Idea", "Research", "Script", "Recording", "Editing", "Thumbnail", "Review", "Sponsor Approval", "Scheduled", "Published"];
  const ROUTES = [
  ...
  const PLATFORMS = ["YouTube", "Shorts", "TikTok", "Instagram Reel", "Carousel", "LinkedIn Post", "Email Teaser"];
  ...
  // Cache top-level DOM anchors once so rendering can stay predictable.
  const els = {
    body: document.body,
  ...
  // Hydrate the last saved workspace snapshot before any route renders.
  let state = loadState();
  // UI state remains separate from persisted workspace data.
  const ui = {
  ...
  let undoStack = [];
  // Walkthrough copy is static and does not need to be recomputed per render.
  ...
  // Normalize saved data, restore the active selections, then paint the UI.
  function init() {
  ...
  function handleClick(event) {
    ...
    let paletteItem = event.target.closest("[data-palette-action]");
    if (paletteItem) {
    ...
    let id = actionButton.getAttribute("data-id");
    let value = actionButton.getAttribute("data-value");
  ...
  function handleSubmit(event) {
    ...
    let data = objectFromForm(new FormData(form));
    let type = form.getAttribute("data-form");
  ...
  function handleChange(event) {
    let target = event.target;
    if (target.id === "contentSelect") {
      ui.selectedContentId = target.value;
      render();
    }
    else if (target.id === "scriptSelect") {
      ui.selectedScriptId = target.value;
      render();
    }
    else if (target.id === "campaignSelect") {
      ui.selectedCampaignId = target.value;
      render();
    }
    else if (target.id === "sponsorSelect") {
      ui.selectedSponsorId = target.value;
      render();
    }
  }
  ...
  function handleDragStart(event) {
    let card = event.target.closest("[data-drag-content]");
    if (!card) return;
  ...
  function handleDragOver(event) {
    let lane = event.target.closest("[data-drop-stage]");
    if (!lane) return;
    event.preventDefault();
  ...
  function handleDrop(event) {
    ...
    if (!lane) return;
    event.preventDefault();
    let contentId = event.dataTransfer.getData("text/plain") || ui.dragContentId;
    let stage = lane.getAttribute("data-drop-stage");
    if (!contentId || !stage) return;
    moveContentToStage(contentId, stage);
  }

  // Rendering lifecycle: route changes update the active view, rebuild the matching module markup, bind contextual UI state, and refresh dashboard metrics without reloading the page.
  function render() {
    ensureSelections();
    renderSidebar();
    renderTopFilters();
  ...
  function renderSidebar() {
    els.sidebarNav.innerHTML = ROUTES.map(function (route) {
      let active = ui.route === route.id ? " is-active" : "";
      let badge = renderRouteBadge(route.id);
      return (
  ...
  function renderRouteBadge(routeId) {
    let badge = getRouteBadgeValue(routeId);
    if (!badge) return "";
    return '<span class="nav-badge ' + (badge.tone || "is-info") + '">' + escapeHtml(badge.label) + "</span>";
  }

    function getRouteBadgeValue(routeId) {
      let metrics = computeDashboardMetrics();
      let insights = routeId === "dashboard" ? computeInsights() : null;
      if (routeId === "dashboard") return insights.length ? { label: insights.length + " alerts", tone: insights[0] ? insights[0].tone : "is-info" } : null;
    if (routeId === "pipeline") return state.content.length ? { label: state.content.filter(function (item) { return item.stage !== "Published"; }).length + " active",
      tone: metrics.blockedCount ? "is-warning" : "is-info" } : null;
      if (routeId === "viral") {
        let weakCount = state.content.filter(function (item) { return computeViralScore(item).total < 70; }).length;
        return weakCount ? { label: weakCount + " weak", tone: "is-warning" } : null;
      }
      if (routeId === "hooks") {
        let favoriteCount = state.hooks.filter(function (item) { return item.favorite; }).length;
    ...
    if (routeId === "sponsors") return state.sponsors.length ? { label: metrics.sponsorRiskCount ? metrics.sponsorRiskCount + " risk" : state.sponsors.length + " deals",
      tone: metrics.sponsorRiskCount ? "is-warning" : "is-success" } : null;
    ...
    function renderTopFilters() {
      let route = getRouteMeta(ui.route);
      ...
      els.assigneeFilter.innerHTML = '<option value="all">All assignees</option>' + state.team.map(function (member) {
        let selected = ui.assigneeFilter === member.id ? ' selected' : "";
    ...
    function getRouteContextSummary(routeId) {
      ...
      if (routeId === "dashboard") {
        let criticalCount = insights.filter(function (item) { return item.severity === "High"; }).length;
      ...
      if (routeId === "viral") return state.content.length ? state.content.filter(function (item) { return computeViralScore(item).total < 70; }).length
        + " items below opportunity target" : "Select content to evaluate packaging strength";
      if (routeId === "hooks") return state.hooks.length ? state.hooks.filter(function (item) { return item.favorite; }).length
        + " favorite angles saved for reuse" : "Generate first-pass hooks from active content briefs";
  ...
  function renderSidebarPulse() {
    ...
    els.sidebarPulse.innerHTML =
      '<div class="pulse-list pulse-list-compact">' +
        ...
        renderPulseItem("Blocked workflows",
          String(metrics.blockedCount),
          metrics.blockedCount === 0 ? "is-success" : metrics.blockedCount >= 3 ? "is-danger" : "is-warning",
          metrics.blockedCount ? "Requires owner attention" : "No active blockers logged") +
        ...
        renderPulseItem("Publishing streak",
          metrics.publishingStreak + " days",
          metrics.publishingStreak >= 3 ? "is-success" : metrics.publishingStreak >= 1 ? "is-warning" : "is-info",
          metrics.weeklyCompletionRate + "% weekly completion") +
      "</div>";
  }
    ...
    els.miniActivity.innerHTML =
      '<div class="mini-activity">' +
        state.activity.slice(0, 4).map(function (item) {
          let tone = inferActivityTone(item);
          return (
  ...
  // Route rendering swaps the main workspace surface without reloading the page.
  ...
  // Dashboard refresh pulls live metrics and insight clusters from current workspace data.
  function renderDashboard() {
    if (!state.content.length) {
      return (
        '<section class="hero-panel">' +
          '<p class="eyebrow">Creator Operations Workspace</p>' +
          ...
          '<div class="hero-actions">' +
            '<button class="button button-primary" data-action="open-content">Create Content Item</button>' +
            '<button class="button button-secondary" data-route="settings">Open Settings</button>' +
          '</div>' +
        '</section>'
      );
    }
    let metrics = computeDashboardMetrics();
    let insights = computeInsights();
    let bottlenecks = computeStagePressure();
    let lowReadiness = state.content.filter(function (item) { return computeReadiness(item).score < 65; }).slice(0, 4);
    let weakOpportunities = state.content.slice().sort(function (a, b) { return computeViralScore(a).total - computeViralScore(b).total; }).slice(0, 4);
    let attentionItems = ui.dashboardQueueView === "opportunity" ? weakOpportunities : lowReadiness;
    let insightGroups = buildInsightClusters(insights);
    let filteredActivity = getDashboardActivity(ui.dashboardFeed).slice(0, 4);
    let topCriticalInsights = insights.slice(0, 3);
    ...
    return (
        ...
        '<div class="hero-command-grid">' +
          '<div class="hero-main">' +
            ...
            '<div class="hero-signal-grid">' +
              renderHeroSignal("Publishing streak",
                metrics.publishingStreak + " days",
                metrics.publishingStreak >= 3 ? "Rhythm stable." : "Cadence pressure building.",
                metrics.publishingStreak >= 3 ? "is-success" : "is-warning") +
              renderHeroSignal("Active launch wave",
                metrics.activeWaveName,
                metrics.activeWaveDeadline ? "Pressure point: " + formatDate(metrics.activeWaveDeadline) : "No active launch pressure.",
                toneByValue(metrics.launchConfidence,
                76,
                58)) +
          ...
          '<aside class="hero-rail">' +
            ...
            '<div class="hero-rail-stack">' +
              renderRailCard("Sponsor pressure",
                metrics.sponsorRiskCount ? metrics.sponsorRiskCount + " deals at risk" : "No sponsor escalation",
                metrics.sponsorRiskCount ? "Approvals are compressing delivery confidence." : "Sponsor delivery is stable.",
                metrics.sponsorRiskCount ? "is-warning" : "is-success") +
              renderRailCard("Workflow blockage",
                metrics.blockedCount ? metrics.blockedCount + " blocked assets" : "No active blockers",
                metrics.blockedCount ? "Dependencies are reducing output speed." : "Critical handoffs are clear.",
                metrics.blockedCount >= 3 ? "is-danger" : metrics.blockedCount ? "is-warning" : "is-success") +
              renderRailCard("Platform dependency",
                metrics.dominantPlatform ? metrics.dominantPlatform + " at " + metrics.dominantPlatformShare + "%" : "Balanced distribution",
                metrics.platformBalanceLabel,
                metrics.dominantPlatformShare >= 45 ? "is-warning" : "is-info") +
            '</div>' +
          '</aside>' +
        '</div>' +
      '</section>' +
      '<section class="metric-grid dashboard-metric-grid">' +
        ...
        renderMetricCard("Platform Balance",
          metrics.platformBalance + "/100",
          metrics.platformBalanceLabel,
          "Channel concentration and distribution resilience.",
          metrics.dominantPlatformShare >= 45 ? "is-warning" : "is-info") +
      '</section>' +
      '<section class="dashboard-layout">' +
        '<div class="panel dashboard-primary-panel">' +
          '<div class="panel-header">' +
            '<div><p class="eyebrow">Smart Insight Engine</p><h3>Grouped operational escalations</h3></div>' +
            '<button class="text-button" data-action="open-notifications">Open center</button>' +
          '</div>' +
          '<div class="dashboard-panel-intro"><strong>'
            + insights.filter(function (item) { return item.severity === "High"; }).length
            + ' critical interventions</strong><span class="helper-text">Workflow, quality, schedule, and commercial pressure.</span></div>' +
        ...
        '<div class="panel dashboard-secondary-panel">' +
          ...
          '<div class="pressure-grid pressure-grid-compact">' +
            ...
            renderPressureCard("Dependencies",
              String(metrics.dependencyRiskCount),
              metrics.dependencyRiskCount ? "Upstream drag active." : "Dependencies controlled.",
              metrics.dependencyRiskCount ? "is-warning" : "is-success") +
          ...
          '<div class="stage-pressure-cluster">' + bottlenecks.slice(0, 3).map(function (item) {
            return '<div class="stage-pressure-row"><div class="stage-pressure-copy"><strong>'
              + escapeHtml(item.stage)
              + '</strong><span class="helper-text">'
              + stagePressureLabel(item.count)
              + '</span></div><div class="stage-pressure-value"><span class="mini-badge">'
              + item.count
              + ' items</span><div class="spark-bar"><span style="width:'
              + Math.min(100, item.count * 18)
              + '%"></span></div></div></div>';
          }).join("") + '</div>' +
        '</div>' +
        '<div class="panel dashboard-timeline-panel">' +
          ...
          '</div>' +
          '<div class="timeline timeline-strong">'
            + (filteredActivity.length ? filteredActivity.slice(0, 4).map(renderTimelineItem).join("") : renderEmpty("No matching feed events", "This filter is clear right now. Switch views or keep the queue moving."))
            + '</div>' +
        '</div>' +
        ...
        '<div class="panel dashboard-watchlist-panel">' +
          '<div class="panel-header"><div><p class="eyebrow">Attention Queue</p><h3>Assets that need intervention next</h3></div><button class="text-button" data-route="'
            + (ui.dashboardQueueView === "opportunity" ? "viral" : "pipeline")
          ...
          '<div class="list-stack">'
            + (attentionItems.length ? attentionItems.map(ui.dashboardQueueView === "opportunity" ? renderOpportunityRow : renderReadinessRow).join("") : renderEmpty("Attention queue is clear",
              ui.dashboardQueueView === "opportunity" ? "No low-opportunity items need packaging intervention right now." : "No low-readiness items are dragging schedule confidence."))
            + '</div>' +
        '</div>' +
      '</section>'
    );
  ...
  function renderDashboardChip(label, value, activeValue, action) {
    let isActive = value === activeValue ? " is-active" : "";
  ...
  function renderPrioritySignal(item) {
    return '<div class="priority-signal '
      + item.tone
      + '"><div class="priority-signal-top"><span class="severity-marker '
      + item.tone
      + '">'
      + escapeHtml(item.severity)
      + '</span><span class="trend-pill">'
      + escapeHtml(item.marker)
      + '</span></div><strong>'
      + escapeHtml(item.title)
      + '</strong><p class="helper-text">'
      + escapeHtml(item.shortReason || item.reason)
      + '</p><button class="button button-ghost" data-action="apply-insight-action" data-id="'
      + item.id
      + '" data-value="'
      + escapeHtml(item.cta || "open-content")
      + '">'
      + escapeHtml(item.primaryLabel || "Open")
      + '</button></div>';
  }
  ...
  function buildInsightClusters(insights) {
    ...
    insights.forEach(function (item) {
      let group = groups.filter(function (entry) { return entry.id === getInsightClusterId(item); })[0] || groups[0];
  ...
  function getInsightClusterId(item) {
    let text = (item.title + " " + item.reason + " " + item.action).toLowerCase();
  ...
  function renderInsightCluster(group) {
    let expanded = ui.dashboardInsightGroup === group.id;
    let preview = group.items.slice(0, expanded ? group.items.length : Math.min(2, group.items.length));
    return (
      '<article class="insight-cluster ' + group.tone + '">' +
        '<button class="insight-cluster-head" data-action="toggle-insight-cluster" data-value="' + group.id + '">' +
          ...
          '</div>' +
          '<div class="insight-cluster-meta"><span class="status-badge '
            + group.tone
            + '">'
            + (group.highCount ? group.highCount + ' critical' : group.items.length + ' open')
            + '</span><span class="cluster-toggle">'
  ...
  function renderPipeline() {
    let grouped = groupByStage(getFilteredContent());
    return (
      renderFilterSummary() +
      '<section class="panel">' +
        '<div class="panel-header">' +
          '<div><p class="eyebrow">Pipeline Overview</p><h3>Drag content across the production system</h3></div>' +
          '<div class="inline-actions">' +
            '<button class="button button-secondary" data-action="open-quick-add">Create Content Item</button>' +
            '<button class="button button-ghost" data-action="open-notifications">Review Blockers</button>' +
          '</div>' +
        '</div>' +
  ...
  function renderViralEngine() {
    let item = getSelectedContent();
    if (!item) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No content available", "Create a content item first to score hook strength, title quality, CTA clarity, retention risk, and platform fit.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="open-content">Create Content Item</button></div></section>';
    }
    let score = computeViralScore(item);
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          ...
          '</div>' +
          '<div class="inline-actions"><button class="button button-primary" data-action="save-viral-result" data-id="'
            + item.id
            + '">Save Result to Content</button><button class="button button-secondary" data-action="build-hook-ideas">Generate Hook Ideas</button></div>' +
        '</div>' +
        '<div class="panel">' +
          ...
          '<div class="panel" style="margin-top:16px;"><p class="eyebrow">Current concept</p><p class="helper-text">'
            + escapeHtml(item.hook || item.title)
            + '</p><p class="eyebrow" style="margin-top:12px;">Improved direction</p><p class="helper-text">'
            + escapeHtml(score.afterExample)
            + '</p></div>' +
        '</div>' +
      '</section>'
    );
  ...
  function renderHooks() {
    let selected = getSelectedContent();
    let liveSeed = ui.selectedHookDraft || (selected ? selected.hook : "");
    let live = analyzeHookDraft(liveSeed);
    let hooks = getFilteredHooks();
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          '<div class="panel-header">' +
            '<div><p class="eyebrow">Live Workspace</p><h3>Draft and score hooks before saving</h3></div>' +
            '<button class="button button-secondary" data-action="open-hook-builder">Save Hook</button>' +
          '</div>' +
          '<div class="field"><label for="liveHookInput">Hook draft</label><textarea id="liveHookInput" data-live-hook placeholder="Write a curiosity, authority, urgency, or story-led hook.">'
            + escapeHtml(liveSeed)
            + '</textarea></div>' +
          '<div class="score-grid">' +
  ...
  function renderScripts() {
    let script = getSelectedScript();
    if (!state.scripts.length) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No scripts saved", state.content.length ? "Create a script to analyze pacing, filler, proof, and CTA strength." : "Create a content item first, then build a script around it.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="'
        + (state.content.length ? 'open-script-editor' : 'open-content')
        + '">'
        + (state.content.length ? 'Create Script' : 'Create Content Item')
        + '</button></div></section>';
    }
    let analysis = analyzeScript(script);
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          '<div class="panel-header">' +
            '<div><p class="eyebrow">Active Script</p><h3>Section structure and clarity</h3></div>' +
            '<div class="inline-actions"><button class="button button-secondary" data-action="open-script-editor">Create Script</button><button class="button button-ghost" data-action="open-script-editor" data-id="' +
            script.id +
            '">Edit Script</button></div>' +
          '</div>' +
  ...
  function renderThumbnailEngine() {
    ...
    if (!item) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No content available", "Create a content item to plan emotional direction, text placement, clutter control, and focal story before design work starts.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="open-content">Create Content Item</button></div></section>';
    }
    let analysis = analyzeThumbnailPlan(item);
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          '<div class="panel-header"><div><p class="eyebrow">Planner</p><h3>Emotional direction and visual hierarchy</h3></div><button class="button button-secondary" data-action="open-thumbnail-planner" data-id="'
            + item.id
            + '">Edit Strategy</button></div>' +
          renderContentSelector("contentSelect", ui.selectedContentId) +
          '<div class="list-stack">' +
  ...
  function renderRepurpose() {
    ...
    if (!item) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No source content available", "Create a content item first so ContentPilot can adapt the core angle into platform-specific versions.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="open-content">Create Content Item</button></div></section>';
    }
    let outputs = buildRepurposePlan(item);
    return (
      renderFilterSummary() +
      '<section class="panel">' +
        ...
        '<div class="repurpose-grid">' + outputs.map(function (entry) {
          return (
            '<div class="asset-card">' +
              ...
              '</ul>' +
              '<div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="copy-repurpose" data-value="'
                + escapeHtml(entry.platform)
                + '">Copy angle</button><button class="button button-ghost" data-action="save-repurpose" data-value="'
                + escapeHtml(entry.platform)
                + '">Save to vault</button></div>' +
            '</div>'
          );
        }).join("") + '</div>' +
      '</section>'
  ...
  function renderCampaigns() {
    let campaigns = getFilteredCampaigns();
    let selected = getSelectedCampaign();
    if (!state.campaigns.length) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No campaigns saved", "Build a campaign to track launch waves, offers, progress, and deadline risk from one place.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="open-campaign">Build Campaign</button></div></section>';
    }
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          '<div class="panel-header">' +
            '<div><p class="eyebrow">Campaign Stack</p><h3>Launches, offers, and publishing waves</h3></div>' +
            '<button class="button button-secondary" data-action="open-campaign">Build Campaign</button>' +
          '</div>' +
          renderCampaignSelector() +
          '<div class="list-stack">'
            + (campaigns.length ? campaigns.map(renderCampaignRow).join("") : renderEmpty("No campaigns match the current filters", "Clear search or filters to review the full campaign stack."))
            + '</div>' +
        '</div>' +
        '<div class="panel">' +
          '<div class="panel-header"><div><p class="eyebrow">Selected Campaign</p><h3>'
            + escapeHtml(selected.name)
            + '</h3></div><div class="inline-actions"><button class="button button-secondary" data-action="open-campaign" data-id="'
            + selected.id
            + '">Edit</button><button class="button button-ghost" data-action="export-report" data-value="campaign">Export report</button></div></div>' +
          '<div class="list-stack">' +
  ...
  function renderSponsors() {
    let sponsors = getFilteredSponsors();
    let sponsor = getSelectedSponsor();
    if (!state.sponsors.length) {
      return renderFilterSummary()
        + '<section class="panel">'
        + renderEmpty("No sponsor deals saved", "Add a sponsor deal to track deliverables, approvals, payment state, and deadline risk.")
        + '<div class="inline-actions" style="margin-top:16px;"><button class="button button-primary" data-action="open-sponsor">Add Sponsor Deal</button></div></section>';
    }
    return (
      renderFilterSummary() +
      '<section class="two-column">' +
        '<div class="panel">' +
          '<div class="panel-header">' +
            '<div><p class="eyebrow">Deal Desk</p><h3>Sponsor deliverables and payment flow</h3></div>' +
            '<button class="button button-secondary" data-action="open-sponsor">Add Sponsor Deal</button>' +
          '</div>' +
          renderSponsorSelector() +
          '<div class="list-stack">'
            + (sponsors.length ? sponsors.map(renderSponsorRow).join("") : renderEmpty("No sponsors match the current filters", "Clear search or filters to review the full sponsor list."))
            + '</div>' +
        '</div>' +
        '<div class="panel">' +
          '<div class="panel-header"><div><p class="eyebrow">Selected Sponsor</p><h3>'
            + escapeHtml(sponsor.name)
            + '</h3></div><div class="inline-actions"><button class="button button-secondary" data-action="open-sponsor" data-id="'
            + sponsor.id
            + '">Edit</button><button class="button button-ghost" data-action="export-report" data-value="sponsor">Export report</button></div></div>' +
          '<div class="list-stack">' +
          ...
          '<div class="inline-actions"><button class="button button-ghost" data-action="toggle-approval" data-id="'
            + sponsor.id
            + '">Advance Approval</button><button class="button button-ghost" data-action="toggle-payment" data-id="'
            + sponsor.id
            + '">Advance Payment</button></div>' +
        '</div>' +
      '</section>'
    );
  }
  ...
  function renderVault() {
    let items = getFilteredVaultItems();
    return (
      renderFilterSummary() +
      '<section class="panel">' +
        '<div class="panel-header">' +
          '<div><p class="eyebrow">Swipe Vault</p><h3>Hooks, CTAs, titles, and creative references</h3></div>' +
          '<button class="button button-secondary" data-action="open-vault-item">Add Vault Item</button>' +
        '</div>' +
    ...
    return (
      ...
      '<section class="two-column">' +
        '<div class="panel">' +
          ...
          '<div class="dashboard-panel-intro"><strong>Operational Pulse™: '
            + escapeHtml(metrics.operationalPulseLabel)
            + '</strong><span class="helper-text">'
            + escapeHtml(metrics.operationalPulseDetail)
            + '</span></div>' +
        ...
        '<div class="panel">' +
          ...
          '<div class="list-stack">'
            + (state.reports.length ? state.reports.slice(0, 8).map(renderReportHistory).join("") : renderEmpty("No exports yet", "Generate an HTML report to populate export history."))
            + '</div>' +
        '</div>' +
      '</section>'
    );
    ...
    return (
      '<section class="panel">' +
        ...
        '<div class="three-column">' +
          '<div class="card"><h4>Theme</h4><p class="helper-text">Current mode: '
            + escapeHtml(state.settings.theme)
            + '</p><button class="button button-secondary" data-action="toggle-theme">Toggle Theme</button></div>' +
          '<div class="card"><h4>Backup & Restore</h4><p class="helper-text">Export or import the complete local workspace safely.</p>' +
            '<div class="inline-actions">' +
              '<button class="button button-secondary" data-action="export-json">Export JSON</button>' +
              '<button class="button button-ghost" data-action="import-json">Import JSON</button>' +
            '</div></div>' +
          '<div class="card"><h4>Onboarding</h4><p class="helper-text">Replay the guided product walkthrough for first-time buyers.</p>' +
            '<button class="button button-secondary" data-action="reset-onboarding">Replay Walkthrough</button>' +
          '</div>' +
        '</div>' +
        '<div class="panel" style="margin-top:18px;">' +
          '<div class="panel-header"><div><p class="eyebrow">Local-first Notes</p><h3>What this product guarantees</h3></div></div>' +
          '<ul class="warning-list">' +
            '<li>All modules work by opening <strong>index.html</strong> directly.</li>' +
            '<li>Data persists in <strong>localStorage</strong> with export, import, overwrite warning, and full reset.</li>' +
            '<li>Reports export as standalone HTML files for stakeholders, clients, or internal archives.</li>' +
          '</ul>' +
        '</div>' +
  ...
  function renderFilterSummary() {
    ...
    return '<section class="panel filter-summary-panel"><div class="panel-header"><div><p class="eyebrow">Active Filters</p><h3>'
      + summary.join(" • ")
      + '</h3></div><button class="button button-ghost" data-action="clear-search">Clear filters</button></div></section>';
  }

  function renderContentCard(item) {
    let readiness = computeReadiness(item);
    let overdue = isOverdue(item.deadline);
    let progress = Math.round(((STAGES.indexOf(item.stage) + 1) / STAGES.length) * 100);
    return (
      '<article class="content-card" draggable="true" data-drag-content="' + item.id + '">' +
        ...
        '<div class="meta-row" style="margin-top:10px;"><span class="mini-badge '
          + (overdue ? "is-danger" : "is-info")
          + '">'
          + formatDate(item.deadline)
          + '</span><span class="mini-badge">Readiness '
          + readiness.score
          + '/100</span></div>' +
        '<div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="open-content" data-id="'
          + item.id
          + '">Open</button><button class="button button-ghost" data-action="create-report-from-content" data-id="'
          + item.id
    ...
    return (
      '<article class="insight-card insight-card-compact ' + item.tone + '">' +
        ...
        '<div class="inline-actions" style="margin-top:10px;">' +
          ...
          (item.secondaryCta
            ? '<button class="button button-ghost" data-action="apply-insight-action" data-id="' + item.id + '" data-value="' + item.secondaryCta + '">' + escapeHtml(item.secondaryLabel) + '</button>'
            : "") +
        '</div>' +
      '</article>'
    );
  ...
  function renderOpportunityRow(item) {
    let score = computeViralScore(item);
    return '<div class="list-item signal-row signal-row-compact"><div class="entity-head"><h4>'
      + escapeHtml(item.title)
      + '</h4><span class="score-badge">'
      + score.total
      + '/100</span></div><div class="helper-text">'
      + escapeHtml(item.platform)
      + ' • '
      + escapeHtml(score.recommendations[0])
      + '</div><div class="inline-actions" style="margin-top:10px;"><button class="button button-ghost" data-action="jump-content-route" data-id="'
      + item.id
      + '" data-value="viral">Generate alternatives</button><button class="button button-ghost" data-action="open-content" data-id="'
      + item.id
      + '">Open workflow</button></div></div>';
  }
  ...
  function renderReadinessRow(item) {
    ...
    return '<div class="list-item signal-row signal-row-compact"><div class="entity-head"><h4>'
      + escapeHtml(item.title)
      + '</h4><span class="readiness-badge '
      + readiness.tone
      + '">'
      + readiness.label
      + '</span></div><div class="helper-text">'
      + escapeHtml(readiness.summary)
      + '</div><div class="inline-actions" style="margin-top:10px;"><button class="button button-ghost" data-action="open-content" data-id="'
      + item.id
      + '">Fix now</button></div></div>';
  }

  function renderHookRow(hook) {
    let analysis = analyzeHookDraft(hook.text);
    return (
      '<div class="vault-item">' +
        ...
        '<div class="inline-actions"><button class="button button-ghost" data-action="load-hook-draft" data-id="'
          + hook.id
          + '">Load draft</button><button class="button button-ghost" data-action="open-hook-builder" data-id="'
          + hook.id
          + '">Edit</button><button class="button button-ghost" data-action="toggle-favorite-hook" data-id="'
          + hook.id
          + '">'
          + (hook.favorite ? "Unfavorite" : "Favorite")
          + '</button><button class="button button-ghost" data-action="duplicate-hook" data-id="'
          + hook.id
          + '">Send to vault</button><button class="button button-ghost" data-action="delete-hook" data-id="'
          + hook.id
          + '">Delete</button></div>' +
      '</div>'
    );
  ...
  function renderCampaignRow(campaign) {
    return '<div class="list-item"><div class="entity-head"><h4>'
      + escapeHtml(campaign.name)
      + '</h4><span class="score-badge">'
      + campaign.progress
      + '%</span></div><div class="helper-text">'
      + escapeHtml(campaign.offer)
      + ' • deadline '
      + formatDate(campaign.deadline)
      + '</div><div class="micro-progress" style="margin-top:10px;"><span style="width:'
      + campaign.progress
      + '%"></span></div><div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="set-selected-campaign" data-id="'
      + campaign.id
      + '">Select</button><button class="button button-ghost" data-action="open-campaign" data-id="'
      + campaign.id
      + '">Open</button></div></div>';
  }
  ...
  function renderSponsorRow(sponsor) {
    return '<div class="list-item"><div class="entity-head"><h4>'
      + escapeHtml(sponsor.name)
      + '</h4><span class="status-badge '
      + approvalClass(sponsor.approval)
      + '">'
      + escapeHtml(sponsor.approval)
      + '</span></div><div class="helper-text">$'
      + sponsor.value.toLocaleString()
      + ' • '
      + escapeHtml(sponsor.paymentStatus)
      + ' • due '
      + formatDate(sponsor.deadline)
      + '</div><div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="set-selected-sponsor" data-id="'
      + sponsor.id
      + '">Select</button><button class="button button-ghost" data-action="open-sponsor" data-id="'
      + sponsor.id
      + '">Open</button></div></div>';
  }

  function renderApprovalRow(item) {
    return '<div class="approval-card"><div class="entity-head"><h4>'
      + escapeHtml(item.title)
      + '</h4><span class="status-badge '
      + approvalClass(item.status)
      + '">'
      ...
      + '</div><div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="toggle-approval" data-id="'
      + item.id
      + '">Advance state</button></div></div>';
  }

  function renderNoteRow(note) {
    return '<div class="team-note"><div class="entity-head"><h4>'
      + escapeHtml(note.title)
      + '</h4><span class="status-badge '
      + (note.done ? "is-success" : "is-warning")
      + '">'
      + (note.done ? "Closed" : "Open")
      + '</span></div><p class="helper-text">'
      + escapeHtml(note.body)
      + '</p><div class="meta-row"><span class="mini-badge">'
      + escapeHtml(getMemberName(note.assigneeId))
      + '</span><span class="mini-badge">'
      + escapeHtml(note.type)
      + '</span></div><div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="complete-note" data-id="'
      + note.id
      + '">'
      + (note.done ? "Reopen" : "Complete")
      + '</button><button class="button button-ghost" data-action="open-team-note" data-id="'
      + note.id
      + '">Edit</button><button class="button button-ghost" data-action="delete-note" data-id="'
      + note.id
      + '">Delete</button></div></div>';
  }

  function renderVaultRow(item) {
    return '<div class="vault-item"><div class="entity-head"><h4>'
      + escapeHtml(item.title)
      + '</h4><span class="mini-badge">'
      + escapeHtml(item.category)
      + '</span></div><p class="helper-text">'
      + escapeHtml(item.content)
      + '</p><div class="meta-row">'
      + item.tags.map(function (tag) { return '<span class="mini-badge">#' + escapeHtml(tag) + '</span>'; }).join("")
      + '<span class="mini-badge">'
      + (item.favorite ? "Favorite" : "Saved")
      + '</span></div><div class="inline-actions" style="margin-top:12px;"><button class="button button-ghost" data-action="toggle-vault-favorite" data-id="'
      + item.id
      + '">'
      + (item.favorite ? "Unfavorite" : "Favorite")
      + '</button><button class="button button-ghost" data-action="open-vault-item" data-id="'
      + item.id
      + '">Edit</button><button class="button button-ghost" data-action="delete-vault" data-id="'
      + item.id
      + '">Delete</button></div></div>';
  }

  function renderReportCard(title, type, body) {
    return '<div class="report-item"><h4>'
      + escapeHtml(title)
      + '</h4><p class="helper-text">'
      + escapeHtml(body)
      + '</p><button class="button button-primary" data-action="export-report" data-value="'
      + type
      + '">Generate HTML</button></div>';
  }
  ...
  function renderCalendarDay(day) {
    ...
    return '<div class="'
      + cls
      + '"><div class="calendar-day-header"><span>'
      + escapeHtml(day.label)
      + '</span><span class="helper-text">'
      + day.events.length
      + '</span></div>'
      + (day.events.length ? day.events.map(function (event) { return '<div class="calendar-event">' + escapeHtml(event) + '</div>'; }).join("") : '<div class="helper-text">No scheduled publish</div>')
      + '</div>';
  }
  ...
  function renderTimelineItem(item) {
    ...
    return '<div class="timeline-item '
      ...
      + escapeHtml(item.title)
      + '</strong><div class="timeline-meta"><span class="mini-badge '
      + tone
      + '">'
      + escapeHtml(getActivityFilterLabel(item))
      + '</span><span class="timeline-badge '
      + tone
      + '">'
      + escapeHtml(item.when)
      + '</span></div></div><div class="helper-text">'
  ...
  function getDashboardActivity(filter) {
    return state.activity.filter(function (item) {
      let kind = getActivityFilterLabel(item).toLowerCase();
  ...
  function getActivityFilterLabel(item) {
    let text = (String(item.title || "") + " " + String(item.detail || "")).toLowerCase();
  ...
  function renderPulseItem(label, value, tone, detail) {
    return '<div class="pulse-item '
      + tone
      + '"><div class="pulse-topline"><span class="severity-dot '
      + tone
      + '"></span><div class="helper-text">'
      + escapeHtml(label)
      ...
      + escapeHtml(value)
      + '</strong>'
      + (detail ? '<div class="helper-text pulse-detail">' + escapeHtml(detail) + '</div>' : "")
      + '</div>';
  }
  ...
  function renderDistributionBars(map) {
    ...
    let total = keys.reduce(function (sum, key) { return sum + map[key]; }, 0);
    return '<div class="distribution-cluster">' + keys.sort(function (a, b) { return map[b] - map[a]; }).map(function (key) {
      let share = Math.round((map[key] / total) * 100);
      let dominant = share >= 45;
      let pressure = dominant ? "Primary load" : share >= 25 ? "Core support" : "Expansion lane";
      return '<div class="distribution-row'
        + (dominant ? ' is-dominant' : '')
        + '"><div class="distribution-head"><div><strong>'
        + escapeHtml(key)
        + '</strong><div class="helper-text">'
        + pressure
        + ' • '
        + map[key]
        + ' active items</div></div><div class="distribution-meta"><span class="mini-badge">'
        + share
        + '% share</span><span class="mini-badge '
        + (dominant ? "is-warning" : "is-info")
        + '">'
        + (dominant ? "Dependency risk" : "Healthy mix")
        + '</span></div></div><div class="spark-bar"><span style="width:'
        + share
        + '%"></span></div></div>';
    }).join("") + '</div>';
  }
  ...
  function renderMetricCard(label, value, trend, body, tone) {
    return '<section class="metric-card '
      + (tone || "")
      + '"><div class="metric-card-top"><p class="eyebrow">'
      + escapeHtml(label)
      + '</p><span class="severity-dot '
      + escapeHtml(tone || "is-info")
      + '"></span></div><strong>'
      + escapeHtml(value)
      + '</strong><div class="metric-trend">'
      + escapeHtml(trend)
      + '</div><p class="metric-footnote">'
      + escapeHtml(body)
      + '</p></section>';
  }

  function renderHeroSignal(label, value, detail, tone) {
    return '<div class="hero-signal '
      + (tone || "is-info")
      + '"><div class="hero-signal-top"><span class="severity-dot '
      + (tone || "is-info")
      + '"></span><span class="hero-signal-label">'
      + escapeHtml(label)
      + '</span></div><strong>'
      + escapeHtml(value)
      + '</strong><p class="helper-text">'
  ...
  function renderRailCard(label, value, detail, tone) {
    return '<div class="rail-card '
      + (tone || "is-info")
      + '"><div class="entity-head"><span class="helper-text">'
      + escapeHtml(label)
  ...
  function renderTopInsight(item) {
    return '<div class="top-insight '
      + item.tone
      + '"><div class="top-insight-head"><span class="severity-marker '
      + item.tone
      + '">'
      + escapeHtml(item.marker)
      + '</span><span class="trend-pill">'
      ...
      + '</h4><p class="helper-text">'
      + escapeHtml(item.shortReason || item.reason)
      + '</p><div class="helper-text"><strong>Next move:</strong> '
      + escapeHtml(item.shortAction || item.action)
      + '</div><div class="inline-actions" style="margin-top:12px;">'
      + (item.cta ? '<button class="button button-primary" data-action="apply-insight-action" data-id="' + item.id + '" data-value="' + item.cta + '">' + escapeHtml(item.primaryLabel) + '</button>' : '')
      +
         (item.secondaryCta
           ? '<button class="button button-ghost" data-action="apply-insight-action" data-id="' + item.id + '" data-value="' + item.secondaryCta + '">' + escapeHtml(item.secondaryLabel) + '</button>'
  ...
  function renderCampaignSelector() {
    ...
    return '<label class="field"><span class="field-title">Selected campaign</span><select id="campaignSelect">' + state.campaigns.map(function (item) {
      let selected = item.id === ui.selectedCampaignId ? ' selected' : "";
  ...
  function renderSponsorSelector() {
    ...
    return '<label class="field"><span class="field-title">Selected sponsor</span><select id="sponsorSelect">' + state.sponsors.map(function (item) {
      let selected = item.id === ui.selectedSponsorId ? ' selected' : "";
      return '<option value="' + item.id + '"' + selected + '>' + escapeHtml(item.name) + '</option>';
    }).join("") + '</select></label>';
  }
  ...
  function renderScriptSelector() {
    ...
    return '<label class="field"><span class="field-title">Selected script</span><select id="scriptSelect">' + state.scripts.map(function (item) {
      let selected = item.id === ui.selectedScriptId ? ' selected' : "";
      return '<option value="' + item.id + '"' + selected + '>' + escapeHtml(item.title) + '</option>';
    }).join("") + '</select></label>';
  }

  function renderPalette() {
    let items = buildPaletteItems().filter(function (item) {
    ...
    els.paletteResults.innerHTML = items.map(function (item) {
      return '<button class="palette-item" data-palette-action="'
        + item.action
        + '" data-value="'
  ...
  function renderNotifications() {
    els.notificationList.innerHTML = getAlerts(true).map(function (item) {
      return '<div class="notification-item"><div class="entity-head"><h4>'
        + escapeHtml(item.title)
        + '</h4><span class="status-badge '
        + item.tone
        + '">'
        ...
        + (item.action ? '<button class="button button-ghost" data-action="apply-insight-action" data-id="' + item.id + '" data-value="' + item.action + '">Open related item</button>' : "")
        + '<button class="button button-ghost" data-action="archive-notification" data-id="'
        + item.id
  ...
  // Modal open/close helpers keep every form and confirmation surface consistent.
  ...
  function openContentModal(id) {
    let item = id ? findById(state.content, id) : null;
    let value = item || createEmptyContent();
  ...
  function openHookBuilderModal(id) {
    let hook = id ? findById(state.hooks, id) : null;
    let selected = getSelectedContent();
    let value = hook || { id: "", text: ui.selectedHookDraft || (selected ? selected.hook : ""), category: "Curiosity", platform: selected ? selected.platform : "YouTube", favorite: false };
    let analysis = analyzeHookDraft(value.text);
    openModal(hook ? "Edit Hook" : "Save Hook", "Hook Laboratory",
      '<form data-form="hook" class="form-grid">' +
        ...
        '<div class="field field-full"><span class="field-title">Live analysis</span><div class="card"><strong>'
          + analysis.total
          + '/100</strong><p class="helper-text">'
  ...
  function openThumbnailPlanner(id) {
    let item = id ? findById(state.content, id) : getSelectedContent();
  ...
  function openCampaignModal(id) {
    let item = id ? findById(state.campaigns, id) : null;
    let value = item || createEmptyCampaign();
  ...
  function openSponsorModal(id) {
    let item = id ? findById(state.sponsors, id) : null;
    let value = item || createEmptySponsor();
  ...
  function openTeamNoteModal(id) {
    let item = id ? findById(state.notes, id) : null;
    let value = item || createEmptyNote();
  ...
  function openVaultModal(id) {
    let item = id ? findById(state.vault, id) : null;
    let value = item || createEmptyVaultItem();
  ...
  function openConfirmModal(title, body, confirmLabel, callback) {
    openModal(title, "Confirmation",
      '<div class="stack"><p class="helper-text">'
        + escapeHtml(body)
        + '</p><div class="inline-actions"><button class="button button-primary" data-action="confirm-modal">'
        + escapeHtml(confirmLabel)
        + '</button><button class="button button-ghost" data-action="close-modal">Cancel</button></div></div>'
    );
    let confirm = els.modalBody.querySelector('[data-action="confirm-modal"]');
  ...
  function renderWalkthrough() {
    let step = walkthroughSteps[ui.walkthroughStep];
  ...
  function saveHook(data) {
    let hookText = String(data.text || "").trim();
    ...
    let duplicate = state.hooks.some(function (hook) {
  ...
  function saveThumbnailPlan(data) {
    mutate("Thumbnail strategy saved", function () {
      let item = getContentById(data.id);
      if (!item) return;
      item.thumbnailPlan = {
  ...
  function moveContentToStage(contentId, stage) {
    let item = getContentById(contentId);
  ...
  function toggleFavoriteHook(id) {
    mutate("Hook updated", function () {
      let hook = findById(state.hooks, id);
      if (!hook) return;
  ...
  function updateApproval(id) {
    mutate("Approval advanced", function () {
      let approval = findById(state.approvals, id);
      if (approval) approval.status = nextApprovalState(approval.status);
      else {
        let sponsor = findById(state.sponsors, id);
  ...
  function toggleTeamNote(id) {
    mutate("Note updated", function () {
      let note = findById(state.notes, id);
      if (!note) return;
      note.done = !note.done;
    });
    render();
  ...
  // Reports are built from the live workspace state, then exported as standalone HTML files.
  function exportReport(type) {
    showLoadingToast("Generating report");
    let report = buildReport(type);
  ...
  function exportWorkspaceJson() {
    let payload = JSON.stringify(Object.assign({ _meta: { product: "ContentPilot", version: "1.0.0", exportedAt: formatStamp(new Date()) } }, state), null, 2);
  ...
  function handleImportFile(event) {
    ...
    reader.onload = function (loadEvent) {
      try {
        let parsed = JSON.parse(String(loadEvent.target.result || "{}"));
        if (!isValidWorkspaceBackup(parsed)) throw new Error("INVALID_WORKSPACE");
        ui.pendingImport = normalizeWorkspace(parsed);
        openModal(
          "Import Workspace",
          "Overwrite Protection",
          '<div class="stack"><p class="helper-text">This backup contains ' +
            ui.pendingImport.content.length +
            ' content items, ' +
            ui.pendingImport.campaigns.length +
            ' campaigns, and ' +
            ui.pendingImport.sponsors.length +
            ' sponsors. Applying it will replace the current workspace.</p>' +
            '<div class="inline-actions"><button class="button button-primary" data-action="apply-import">Apply Import</button><button class="button button-ghost" data-action="close-modal">Cancel</button></div></div>'
        );
      } catch (error) {
  ...
  // Import only replaces the active workspace after validation and overwrite confirmation.
  ...
  // Persist the normalized workspace after each mutation so refreshes keep the latest state.
  function saveState() {
    try {
  ...
  // Load the last saved workspace snapshot or fall back to the seeded demo environment.
  function loadState() {
    try {
  ...
  // Normalize imported or stored data into the structure expected by the rendering layer.
  function normalizeWorkspace(workspace) {
    let seed = createSeedState();
    let merged = workspace && typeof workspace === "object" ? workspace : {};
    return {
  ...
  // Keep route selectors pointed at valid records after imports, deletes, and resets.
  ...
  function getSelectedContent() {
    return findById(state.content, ui.selectedContentId) || state.content[0] || null;
  }
  function getSelectedScript() {
    return findById(state.scripts, ui.selectedScriptId) || state.scripts[0] || null;
  }
  function getSelectedCampaign() {
    return findById(state.campaigns, ui.selectedCampaignId) || state.campaigns[0] || null;
  }
  function getSelectedSponsor() {
    return findById(state.sponsors, ui.selectedSponsorId) || state.sponsors[0] || null;
  }
  function getContentById(id) {
    return findById(state.content, id);
  }

  function getFilteredContent() {
    return state.content.filter(function (item) {
      let haystack = [item.title, item.description, item.platform, item.stage, item.hook, item.cta, item.topic].join(" ").toLowerCase();
      let searchMatch = !ui.search || haystack.indexOf(ui.search) > -1;
      let assigneeMatch = ui.assigneeFilter === "all" || item.assigneeId === ui.assigneeFilter;
      let viewMatch = matchesView(item);
  ...
  function computeDashboardMetrics() {
    ...
    let readinessAverage = average(content.map(function (item) { return computeReadiness(item).score; }));
    let viralAverage = average(content.map(function (item) { return computeViralScore(item).total; }));
    let overdue = content.filter(function (item) { return isOverdue(item.deadline) && item.stage !== "Published"; }).length;
    let published = content.filter(function (item) { return item.stage === "Published"; }).length;
    let scheduled = content.filter(function (item) { return item.stage === "Scheduled"; }).length;
    let reviewStuck = content.filter(function (item) { return item.stage === "Review" || item.stage === "Sponsor Approval"; }).length;
    let blockedCount = content.filter(function (item) { return Boolean(item.blocker); }).length;
    let lowReadinessCount = content.filter(function (item) { return computeReadiness(item).score < 70; }).length;
    let weakHookCount = content.filter(function (item) { return analyzeHookDraft(item.hook).total < 68; }).length;
    let weakThumbnailCount = content.filter(function (item) { return analyzeThumbnailPlan(item).score < 70; }).length;
    let missingCtaCount = content.filter(function (item) { return !item.cta || item.cta.length < 12; }).length;
    let dependencyRiskCount = content.filter(function (item) { return item.dependencies && item.dependencies.length >= 2 && item.stage !== "Published"; }).length;
    let urgentDeadlineCount = content.filter(function (item) { return dateDiffDays(item.deadline) <= 2 && item.stage !== "Published"; }).length;
    let consistencyScore = clampNumber((published * 12) + (scheduled * 7), 0, 100);
    ...
    let platformCounts = Object.keys(platformDistribution).map(function (key) { return platformDistribution[key]; });
    let concentration = platformCounts.length ? Math.max.apply(null, platformCounts) / content.length : 0;
    let platformBalance = clampNumber(100 - Math.round(concentration * 42), 35, 96);
    let dominantPlatform = getDominantPlatform(platformDistribution);
    let dominantPlatformShare = dominantPlatform ? Math.round((platformDistribution[dominantPlatform] / content.length) * 100) : 0;
    let sponsorRiskCount = state.sponsors.filter(function (item) { return item.approval !== "Approved" && dateDiffDays(item.deadline) <= 4; }).length;
    let campaignMomentum = clampNumber(
    ...
    let publishingStreak = computePublishingStreak();
    let weeklyCompletionRate = clampNumber(((published + scheduled) / Math.max(1, content.length)) * 100, 0, 100);
    let productionMomentum = clampNumber(average(content.map(function (item) { return ((STAGES.indexOf(item.stage) + 1) / STAGES.length) * 100; })) - (overdue * 7) - (blockedCount * 4) + (scheduled * 4), 12, 100);
    let activeWave = getActiveCampaignSummary();
    let burnoutRisk = clampNumber((overdue * 13) + (reviewStuck * 9) + (blockedCount * 10) + (urgentDeadlineCount * 12), 0, 100);
    let launchConfidence = clampNumber(
      average([
        campaignMomentum,
        readinessAverage,
        viralAverage,
    ...
    let healthScore = clampNumber((readinessAverage * 0.34) + (viralAverage * 0.26) + (consistencyScore * 0.18) + (platformBalance * 0.1) + ((100 - overdue * 8) * 0.12), 22, 98);
    let pulse = computeOperationalPulseState({
    ...
    return {
      ...
      productionMomentum: productionMomentum,
      productionMomentumLabel: productionMomentum >= 76
        ? "Production is moving with confidence."
        : productionMomentum >= 58
          ? "Production is moving, but pressure is building."
          : "Production drag is slowing weekly output.",
  ...
  function computeReadiness(item) {
    let score = 0;
    let notes = [];
    let script = state.scripts.filter(function (entry) { return entry.contentId === item.id; })[0];
    if (script && analyzeScript(script).checkpoints >= 5) score += 20; else notes.push("Script structure is incomplete.");
    let hookScore = analyzeHookDraft(item.hook).total;
    if (hookScore >= 75) score += 15; else notes.push("Hook quality needs work.");
    let thumbnailScore = analyzeThumbnailPlan(item).score;
    if (item.thumbnailPlan && item.thumbnailPlan.focus && item.thumbnailPlan.direction && thumbnailScore >= 72) {
      score += 12;
    } else {
      notes.push(item.thumbnailPlan ? "Thumbnail clarity is soft." : "Thumbnail strategy is incomplete.");
    }
    ...
    let sponsor = item.sponsorId ? findById(state.sponsors, item.sponsorId) : null;
    ...
    let label = score >= 85 ? "Ready to Publish" : score >= 70 ? "Almost Ready" : score >= 50 ? "Needs Work" : "Not Ready";
    let tone = score >= 85 ? "is-success" : score >= 70 ? "is-info" : score >= 50 ? "is-warning" : "is-danger";
  ...
  function computePublishingStreak() {
    ...
    for (let index = 0; index < 10; index += 1) {
      let date = todayOffset(index * -1);
      let hasPublish = state.content.some(function (item) {
  ...
  function getActiveCampaignSummary() {
    let active = state.campaigns.slice().sort(function (a, b) {
  ...
  function computeInsights() {
    ...
    state.content.forEach(function (item) {
      let hookScore = analyzeHookDraft(item.hook).total;
      let readiness = computeReadiness(item);
      let viral = computeViralScore(item);
      let thumbnail = analyzeThumbnailPlan(item);
      if (hookScore < 68) {
        insights.push(makeInsight("insight-hook-" + item.id,
          "Hook is underperforming",
          "High",
          "The current opener lacks enough tension or curiosity for " + item.title + ".",
          "Strengthen the opening contrast or generate three new angles.",
          "open-content",
          item.id,
          { trend: "Opening drag",
          marker: "Hook risk" }));
      ...
      if (!item.cta || item.cta.length < 12) {
        insights.push(makeInsight("insight-cta-" + item.id,
          "CTA is missing",
          "Medium",
          item.title + " does not have a strong next-step CTA.",
          "Add a platform-specific action that converts attention into a result.",
          "open-content",
          item.id,
          { trend: "Conversion softness",
          marker: "CTA risk" }));
      }
      if (isOverdue(item.deadline) && item.stage !== "Published") {
        insights.push(makeInsight("insight-overdue-" + item.id,
          "Production item is overdue",
          "High",
          item.title + " is still in " + item.stage + " after its deadline.",
          "Reprioritize or move the deadline after confirming the blocker.",
          "open-content",
          item.id,
          { trend: "Deadline slip",
          marker: "Schedule risk" }));
      }
      if (viral.total < 70) {
        insights.push(makeInsight("insight-viral-" + item.id,
          "Opportunity score is low",
          "Medium",
          item.title + " is below the target opportunity threshold.",
          "Use the Viral Opportunity Engine to improve title, hook, CTA, or thumbnail clarity.",
          "route-viral",
          item.id,
          { trend: "Packaging drag",
          marker: "Opportunity risk" }));
      }
      if (readiness.score < 60) {
        insights.push(makeInsight("insight-ready-" + item.id,
          "Content readiness is low",
          "Medium",
          item.title + " is not ready for scheduling.",
          "Complete script, CTA, thumbnail plan, or campaign assignment before moving forward.",
          "open-content",
          item.id,
          { trend: "Readiness drag",
          marker: "Readiness risk" }));
      }
      if (thumbnail.score < 70) {
        insights.push(makeInsight("insight-thumbnail-" + item.id,
          "Thumbnail confidence is weak",
          "Medium",
          item.title + " is carrying a thumbnail plan that may not convert attention cleanly.",
          "Simplify the focal story and remove the main visual distraction before scheduling.",
          "open-content",
          item.id,
          { trend: "Packaging drag",
          marker: "Thumbnail risk" }));
      }
      if (item.blocker) {
        insights.push(makeInsight("insight-blocker-" + item.id,
          "Workflow blocker is active",
          dateDiffDays(item.deadline) <= 2 ? "High" : "Medium",
          item.title + " is carrying an active blocker: " + item.blocker,
          "Review the blocker, confirm the dependency owner, and decide whether this item should move or be rescheduled.",
          "open-content",
          item.id,
          { trend: "Flow interruption",
          marker: "Workflow risk" }));
      }
    });
    state.sponsors.forEach(function (item) {
      if (item.approval !== "Approved" && dateDiffDays(item.deadline) <= 4) {
        insights.push(makeInsight("insight-sponsor-" + item.id,
          "Sponsor deliverable risk",
          "High",
          item.name + " still needs approval with a close deadline.",
          "Move the sponsor record forward or realign the delivery promise today.",
          "open-sponsor",
          item.id,
          { trend: "Commercial exposure",
          marker: "Sponsor risk" }));
      }
    });
    let reviewStuck = state.content.filter(function (item) { return item.stage === "Review" || item.stage === "Sponsor Approval"; });
    if (reviewStuck.length >= 3) {
      insights.push(makeInsight("insight-review-queue",
        "Too many items are stuck in review",
        "High",
        reviewStuck.length + " items are sitting in review-related stages.",
        "Clear one approval lane before adding new content to the queue.",
        "route-pipeline",
        "",
        { trend: "Review congestion",
        marker: "Queue risk" }));
    }
    if (metrics.platformBalance < 62) {
      insights.push(makeInsight("insight-platform-balance",
        "Platform mix is unbalanced",
        "Medium",
        "Too much of the workload is concentrated on one platform.",
        "Shift one upcoming asset into a weaker channel to improve distribution resilience.",
        "route-calendar",
        "",
        { trend: "Distribution concentration",
        marker: "Platform risk" }));
    }
    if (countPublishingGaps() >= 4) {
      insights.push(makeInsight("insight-gap",
        "Publishing gaps detected",
        "Medium",
        "The next 14 days include multiple empty publishing days.",
        "Pull one high-readiness item into Scheduled to protect consistency.",
        "route-calendar",
        "",
        { trend: "Cadence risk",
        marker: "Publishing gap" }));
    }
    if (metrics.weeklyCompletionRate < 55) {
      insights.push(makeInsight("insight-slowdown",
        "Publishing slowdown is forming",
        "Medium",
        "Weekly completion is below the target operational threshold and consistency may slip.",
        "Move one almost-ready asset into a scheduled slot and clear the next blocker in line.",
        "route-pipeline",
        "",
        { trend: "Output slowdown",
        marker: "Momentum risk" }));
    }
    if (metrics.launchConfidence < 60) {
      insights.push(makeInsight("insight-launch-confidence",
        "Launch confidence is deteriorating",
        "High",
        "Current campaign readiness, sponsor pressure, and delivery drag are weakening launch confidence.",
        "Stabilize the active wave before adding new work into the release queue.",
        "route-pipeline",
        "",
        { trend: "Launch instability",
        marker: "Launch risk",
        primaryLabel: "Open affected workflow" }));
    }
    if (metrics.burnoutRisk >= 55) {
      insights.push(makeInsight("insight-burnout",
        "Burnout risk is rising",
        metrics.burnoutRisk >= 72 ? "High" : "Medium",
        "Overdue work, review drag, and urgent deadlines are increasing execution fatigue.",
        "Reduce one pressure point now by rescheduling a weak asset or clearing a stuck approval.",
        "route-calendar",
        "",
        { trend: "Workload strain",
        marker: "Burnout risk",
        primaryLabel: "Reschedule dependency" }));
    }
    getRepeatedTopics().forEach(function (topic) {
      insights.push(makeInsight("insight-topic-" + topic,
        "Repeated content topic",
        "Low",
        "The topic \"" + topic + "\" appears repeatedly in the active queue.",
        "Vary the angle, audience, or promise to avoid creative overlap.",
        "route-pipeline",
        ""));
    });
    state.campaigns.forEach(function (item) {
      if (item.progress < 55 && dateDiffDays(item.deadline) <= 6) {
        insights.push(makeInsight("insight-campaign-" + item.id,
          "Campaign readiness is low",
          "High",
          item.name + " is behind pace for its current deadline.",
          "Tighten the sequence, remove one weak asset, and push the hero piece first.",
          "open-campaign",
          item.id,
          { trend: "Wave delay",
          marker: "Campaign risk" }));
      }
    });
  ...
  function makeInsight(id, title, severity, reason, action, cta, relatedId, options) {
    let tone = severity === "High" ? "is-danger" : severity === "Medium" ? "is-warning" : "is-info";
    let meta = buildInsightMeta(title, severity, cta, relatedId, options || {});
    return {
      id: id,
      title: title,
      severity: severity,
  ...
  function applyInsightAction(id, action) {
    let insight = computeInsights().filter(function (item) { return item.id === id; })[0];
    if (!insight) return;
  ...
  function computeViralScore(item) {
    let hookAnalysis = analyzeHookDraft(item.hook);
    let titleScore = analyzeTitle(item.title);
    let ctaScore = analyzeCta(item.cta);
    let retentionScore = analyzeRetention(item.description, item.platform);
    let fitScore = platformMatchScore(item.platform, item.stage);
    let thumb = analyzeThumbnailPlan(item).score;
    let breakdown = [
    ...
    let total = Math.round(average(breakdown.map(function (entry) { return entry.score; })));
  ...
  function analyzeHookDraft(text) {
    ...
    let curiosity = containsAny(text, ["why", "how", "nobody", "secret", "mistake", "truth", "before", "after"]) ? 88 : clampNumber(42 + words.length * 3, 35, 70);
    let authority = containsAny(text, ["tested", "proven", "spent", "built", "scaled", "clients", "revenue", "system"]) ? 84 : 58;
    let urgency = containsAny(text, ["today", "now", "before", "stop", "risk", "too late"]) ? 82 : 54;
    let clarity = text.length >= 18 && text.length <= 150 ? 78 : 58;
    let emotion = containsAny(text, ["stuck", "wasting", "embarrassing", "painful", "obsessed", "fear", "late", "chaos"]) ? 84 : 57;
    let total = Math.round(average([curiosity, authority, urgency, clarity, emotion]));
    let notes = [];
  ...
  function analyzeTitle(title) {
    ...
    let specificity = clean.length >= 28 && clean.length <= 72 ? 80 : 60;
    let tension = containsAny(clean, ["why", "how", "mistake", "truth", "hidden", "cost", "secret", "saved", "killed"]) ? 84 : 62;
    let clarity = words.length >= 5 && words.length <= 12 ? 78 : 64;
    return clampNumber(average([specificity, tension, clarity]), 42, 95);
  }

  function analyzeCta(cta) {
    let clean = String(cta || "").trim();
    if (!clean) return 28;
    let specificity = containsAny(clean, ["download", "save", "comment", "reply", "watch", "grab", "share", "audit"]) ? 82 : 58;
    let clarity = clean.length >= 12 ? 76 : 52;
    let outcome = containsAny(clean, ["checklist", "framework", "newsletter", "template", "system", "guide"]) ? 80 : 60;
  ...
  function analyzeRetention(description, platform) {
    ...
    let structure = containsAny(clean, ["breakdown", "framework", "checklist", "mistakes", "teardown", "walkthrough"]) ? 80 : 62;
    let payoff = containsAny(clean, ["saved", "fixed", "recovered", "improve", "scale", "stop"]) ? 78 : 60;
    let pace = platform === "Shorts" || platform === "TikTok" || platform === "Instagram Reel" ? 76 : 72;
    return clampNumber(average([structure, payoff, pace]), 40, 92);
  }

  function analyzeScript(script) {
    let all = Object.keys(script.sections).map(function (key) { return script.sections[key]; }).join(" ");
    let words = all.split(/\s+/).filter(Boolean).length;
    let fillerTerms = ["just", "really", "actually", "basically", "kind of", "sort of", "maybe", "literally"];
    let fillerWarnings = fillerTerms.filter(function (term) { return all.toLowerCase().indexOf(term) > -1; });
    let checkpoints = Object.keys(script.sections).filter(function (key) { return script.sections[key] && script.sections[key].length > 30; }).length;
    let pacing = words < 180 ? "Fast" : words < 340 ? "Balanced" : "Dense";
    let retention = clampNumber(82 - fillerWarnings.length * 8 - (pacing === "Dense" ? 10 : 0) + checkpoints * 2, 42, 95);
    let notes = [];
  ...
  function buildRepurposePlan(item) {
    return Object.keys(PLATFORM_GUIDES).map(function (platform) {
      let guide = PLATFORM_GUIDES[platform];
      let score = clampNumber(computeViralScore(item).total - (platform === item.platform ? 0 : 6) + ((platform === "Shorts" || platform === "TikTok") ? 2 : 0), 58, 95);
      return {
        platform: platform,
        score: score,
  ...
  function buildCalendar() {
    ...
    return Array.from({ length: 14 }).map(function (_, index) {
      ...
      let label = current.toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" });
      let events = state.content.filter(function (item) { return item.deadline === isoDate(current); }).map(function (item) {
  ...
  function buildInsightMeta(title, severity, cta, relatedId, options) {
    ...
    let content = findById(state.content, relatedId);
    let sponsor = findById(state.sponsors, relatedId);
    let campaign = findById(state.campaigns, relatedId);
    let lowerTitle = String(title || "").toLowerCase();

    if (content) {
      let days = dateDiffDays(content.deadline);
      ...
      let sponsorDays = dateDiffDays(sponsor.deadline);
      ...
      let campaignDays = dateDiffDays(campaign.deadline);
  ...
  function buildReport(type) {
    let metrics = computeDashboardMetrics();
    let insights = computeInsights().slice(0, 6);
    let titleMap = {
    ...
    let title = titleMap[type] || "ContentPilot Report";
    let filename = title.toLowerCase().replace(/\s+/g, "-") + ".html";
    let summary = "";
    let table = "";
    let headings = [];
    ...
    let risks = insights.map(function (item) {
      return '<li><span class="report-list-label '
        + item.tone
        + '">'
        + escapeHtml(item.marker)
        + '</span><div><strong>'
    ...
    let recs = insights.map(function (item) {
    ...
    return {
      ...
      html: buildHtmlReportDocument(title,
        '<section class="report-hero"><p class="report-kicker">Operational export</p><h2>'
          + escapeHtml(title)
          + '</h2><p>'
          + escapeHtml(metrics.operationalPulseDetail)
          + '</p><div class="report-hero-meta"><span>Generated '
          + escapeHtml(formatStamp(new Date()))
          + '</span><span>Local workspace export</span></div></section>' +
        metricsGrid +
        "<section><h2>Executive Summary</h2>" + summary + "</section>" +
        "<section><h2>Operational Snapshot</h2><ul class=\"report-bullet-grid\"><li><strong>Content Health Score</strong><span>"
          + metrics.healthScore
          + "/100</span></li><li><strong>Publishing Consistency</strong><span>"
          + escapeHtml(metrics.consistencyLabel)
          + "</span></li><li><strong>Overdue Production Items</strong><span>"
          + metrics.overdueCount
          + "</span></li><li><strong>Sponsor Risk Alerts</strong><span>"
          + metrics.sponsorRiskCount
          + "</span></li></ul></section>" +
  ...
  function buildSingleContentReport(item) {
    let readiness = computeReadiness(item);
    let viral = computeViralScore(item);
    let repurpose = buildRepurposePlan(item).slice(0, 4);
    return buildHtmlReportDocument(item.title + " Content Plan",
      '<section class="report-hero"><p class="report-kicker">Content export</p><h2>'
        + escapeHtml(item.title)
        + '</h2><p>'
        + escapeHtml(item.description)
        + '</p><div class="report-hero-meta"><span>'
        + escapeHtml(item.platform)
        + '</span><span>'
        + escapeHtml(item.stage)
        + '</span><span>'
      ...
      "<section><h2>Opportunity Score</h2><p>"
        + viral.total
        + "/100</p><ul class=\"report-recommendations\">"
        + viral.recommendations.map(function (entry) { return "<li>" + escapeHtml(entry) + "</li>"; }).join("")
        + "</ul></section>" +
      "<section><h2>Repurposing Recommendations</h2><ul>"
        + repurpose.map(function (entry) { return "<li><strong>" + escapeHtml(entry.platform) + ":</strong> " + escapeHtml(entry.angle) + "</li>"; }).join("")
        + "</ul></section>"
    );
  }
  ...
  function buildHtmlReportDocument(title, body) {
    return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>' +
      escapeHtml(title) +
      '</title><style>' +
      'body{margin:0;font-family:Segoe UI,Tahoma,sans-serif;background:#eef3fb;color:#14203a;padding:36px}' +
      'main{max-width:1040px;margin:0 auto;background:#fff;border-radius:30px;padding:42px;box-shadow:0 24px 70px rgba(17,32,61,.12)}' +
      'h1,h2{letter-spacing:-.04em;margin:0 0 10px}h2{font-size:1.4rem}p{line-height:1.68}' +
      'section{margin-top:26px;padding-top:18px;border-top:1px solid #e7edf7}.eyebrow,.report-kicker{font-size:12px;text-transform:uppercase;letter-spacing:.18em;color:#7384a6;margin:0 0 8px}' +
      '.report-hero{margin-top:0;padding-top:0;border-top:none;padding:28px;border-radius:24px;background:linear-gradient(135deg,#0f1b35,#15284c 55%,#1f3568);color:#f4f8ff}' +
      '.report-hero h2{font-size:2.05rem;margin-bottom:8px}.report-hero p{max-width:760px;color:#d9e6ff}' +
      '.report-hero-meta{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}' +
      '.report-hero-meta span{display:inline-flex;align-items:center;padding:8px 12px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);font-size:.82rem}' +
      '.report-metric-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-top:22px}' +
      '.report-metric{padding:18px;border:1px solid #e7edf7;border-radius:20px;background:#f8fbff}' +
      '.report-metric span{display:block;color:#66789d;font-size:.78rem;text-transform:uppercase;letter-spacing:.12em}' +
      '.report-metric strong{display:block;margin-top:10px;font-size:1.3rem;letter-spacing:-.03em}' +
      '.report-metric p{margin:10px 0 0;color:#445371;font-size:.95rem}' +
      '.report-bullet-grid,.report-risk-list,.report-recommendations{list-style:none;padding:0;margin:0;display:grid;gap:12px}' +
      '.report-bullet-grid{grid-template-columns:repeat(2,minmax(0,1fr))}' +
      '.report-bullet-grid li,.report-risk-list li,.report-recommendations li{padding:14px 16px;border-radius:18px;background:#f8fbff;border:1px solid #e7edf7}' +
      '.report-bullet-grid li{display:flex;justify-content:space-between;gap:16px;align-items:center}' +
      '.report-bullet-grid li span{font-weight:700}' +
      '.report-risk-list li{display:grid;grid-template-columns:auto 1fr;gap:12px;align-items:flex-start}' +
      '.report-risk-list li p{margin:6px 0 0;color:#445371}' +
      '.report-list-label{display:inline-flex;align-items:center;justify-content:center;padding:7px 10px;border-radius:999px;font-size:.74rem;text-transform:uppercase;letter-spacing:.08em}' +
      '.report-list-label{border:1px solid #e7edf7;background:#fff;height:max-content}' +
      '.report-list-label.is-danger{color:#c54969;border-color:rgba(197,73,105,.18);background:#fff4f7}' +
      '.report-list-label.is-warning{color:#b17b16;border-color:rgba(177,123,22,.18);background:#fff9ef}' +
      '.report-list-label.is-info{color:#3468d0;border-color:rgba(52,104,208,.18);background:#f3f7ff}' +
      'ul{padding-left:20px;line-height:1.7}' +
      'table{width:100%;border-collapse:collapse;margin-top:10px}' +
      'th,td{padding:12px 10px;border-bottom:1px solid #e8eef8;vertical-align:top;text-align:left;overflow-wrap:anywhere}' +
      'th{font-size:12px;text-transform:uppercase;letter-spacing:.12em;color:#66789d;background:#f7f9fd}' +
      '@media (max-width:860px){body{padding:18px}main{padding:24px}.report-metric-grid,.report-bullet-grid{grid-template-columns:1fr}.report-risk-list li{grid-template-columns:1fr}}' +
      '@media print{body{padding:0;background:#fff}main{box-shadow:none;border-radius:0;padding:28px}' +
      '.report-hero{background:#fff;color:#14203a;border:1px solid #e7edf7}.report-hero p{color:#445371}.report-hero-meta span{background:#fff;border-color:#e7edf7}}' +
      '</style></head><body><main><p class="eyebrow">Generated by ContentPilot</p><h1>'
      + escapeHtml(title)
      + '</h1>'
      + body
      + '</main></body></html>';
  ...
  function generateHookIdeasForSelected() {
    ...
    openModal("Generated Hook Ideas", "Hook Laboratory", '<div class="stack">' + ui.generatedHookIdeas.map(function (idea) {
      return '<div class="list-item"><h4>'
        + escapeHtml(idea.text)
        + '</h4><div class="inline-actions"><button class="button button-ghost" data-action="load-generated-hook" data-id="'
        + idea.id
        + '">Load draft</button><button class="button button-ghost" data-action="save-generated-hook" data-id="'
        + idea.id
        + '">Save to lab</button></div></div>';
    }).join("") + '</div>');
  }

  function loadGeneratedHook(index) {
    let idea = ui.generatedHookIdeas[index];
    if (!idea) return;
  ...
  function copyRepurposeAngle(platform) {
    ...
    let plan = buildRepurposePlan(item).filter(function (entry) { return entry.platform === platform; })[0];
    if (!plan) return;
    copyText(plan.angle);
  }
  ...
  function toggleVaultFavorite(id) {
    mutate("Vault item updated", function () {
      let item = findById(state.vault, id);
      if (!item) return;
  ...
  function jumpToLowestScoringItem() {
    let candidate = state.content.slice().sort(function (a, b) { return computeViralScore(a).total - computeViralScore(b).total; })[0];
  ...
  function confirmDeleteContent(id) {
    let item = findById(state.content, id);
    if (!item) return;
  ...
  function confirmDeleteHook(id) {
    let item = findById(state.hooks, id);
    if (!item) return;
  ...
  function confirmDeleteScript(id) {
    let item = findById(state.scripts, id);
    if (!item) return;
  ...
  function confirmDeleteCampaign(id) {
    let item = findById(state.campaigns, id);
    if (!item) return;
  ...
  function confirmDeleteSponsor(id) {
    let item = findById(state.sponsors, id);
    if (!item) return;
  ...
  function confirmDeleteNote(id) {
    let item = findById(state.notes, id);
    if (!item) return;
  ...
  function isValidWorkspaceBackup(parsed) {
    ...
    let required = ["team", "content", "hooks", "scripts", "campaigns", "sponsors", "approvals", "notes", "vault", "settings"];
  ...
  function toast(title, detail) {
    ...
    node.innerHTML = "<strong>"
      + escapeHtml(title)
      + '</strong><div class="helper-text" style="margin-top:6px;">'
      + escapeHtml(detail)
      + '</div><div class="inline-actions" style="margin-top:10px;"><button class="button button-ghost" data-action="dismiss-toast" data-id="'
      + id
      + '">Dismiss</button></div>';
  ...
  function dismissToast(id) {
    let toastNode = els.toastStack.querySelector('[data-toast-id="' + id + '"]');
    if (toastNode && toastNode.parentNode) toastNode.parentNode.removeChild(toastNode);
  }

  function downloadTextFile(filename, content, mimeType) {
    let blob = new Blob([content], { type: mimeType || "text/plain;charset=utf-8" });
  ...
  function fieldSelect(label, name, options, value) {
    let normalized = options.map(function (option) {
  ...
  function formActions(label, secondaryHtml) {
    return '<div class="field field-full"><div class="inline-actions"><button class="button button-primary" type="submit">'
      + escapeHtml(label)
      + '</button>'
      + (secondaryHtml || "")
      + '<button class="button button-ghost" type="button" data-action="close-modal">Cancel</button></div></div>';
  }
  ...
  function normalizeContentInput(data) {
    let existing = data.id ? findById(state.content, data.id) : null;
  ...
  function createSeedState() {
    return {
      ...
      content: [
        { id: "content-1",
          title: "YouTube Launch: The Creator Ops System That Saved Our Weekly Publish",
          description: "Hero launch video for a creator operations audit offer, built around workflow chaos and operational recovery.",
          platform: "YouTube",
          stage: "Editing",
          deadline: todayOffset(2),
          priority: "Critical",
          assigneeId: "team-3",
          topic: "creator operations systems",
          dependencies: ["Sponsor notes",
          "Thumbnail direction"],
          blocker: "Awaiting final b-roll exports and legal note integration.",
          hook: "Most creator teams are not losing consistency because of bad ideas. They are losing because the operation behind the content is broken.",
          cta: "Download the weekly creator ops checklist from the newsletter.",
          campaignId: "campaign-1",
          sponsorId: "sponsor-1",
          lastOpportunityScore: 74,
          thumbnailPlan: { direction: "Urgent",
          textPlacement: "Minimal text",
          clutter: "Balanced",
          focus: "Before/after" } },
        { id: "content-2",
          title: "TikTok Sprint: Why Most Podcast Clips Die Before 5 Seconds",
          description: "Short-form retention breakdown for podcast studios and creator teams repurposing long-form shows.",
          platform: "TikTok",
          stage: "Thumbnail",
          deadline: todayOffset(1),
          priority: "High",
          assigneeId: "team-2",
          topic: "podcast clip retention",
          dependencies: ["Captions",
          "Sound cleanup"],
          blocker: "",
          hook: "If your clip needs context before the payoff, the swipe already happened.",
          cta: "Save this and audit your next three clips tonight.",
          campaignId: "campaign-2",
          sponsorId: "",
          lastOpportunityScore: 82,
          thumbnailPlan: { direction: "Shock",
          textPlacement: "Left focus",
          clutter: "Minimal",
          focus: "Face reaction" } },
        { id: "content-3",
          title: "Instagram Reel Series: The Hidden Cost of Late Sponsor Approval",
          description: "Operational mini-series connecting delayed sponsor reviews with missed publishing windows and stressed teams.",
          platform: "Instagram Reel",
          stage: "Sponsor Approval",
          deadline: todayOffset(-1),
          priority: "Critical",
          assigneeId: "team-1",
          topic: "sponsor approval delays",
          dependencies: ["Brand script signoff"],
          blocker: "Sponsor legal team requested revised mention language.",
          hook: "A slow sponsor approval does not only delay one reel. It can wreck the entire publishing week behind it.",
          cta: "Share this with the person who keeps chasing sponsor approvals manually.",
          campaignId: "campaign-1",
          sponsorId: "sponsor-2",
          lastOpportunityScore: 68,
          thumbnailPlan: { direction: "Urgent",
          textPlacement: "Right focus",
          clutter: "Balanced",
          focus: "Object contrast" } },
        { id: "content-4",
          title: "LinkedIn Breakdown: How Agencies Turn One Client Shoot Into 12 Assets",
          description: "B2B distribution case study for agencies building content engines from a single shoot day.",
          platform: "LinkedIn Post",
          stage: "Scheduled",
          deadline: todayOffset(4),
          priority: "Medium",
          assigneeId: "team-4",
          topic: "content repurposing systems",
          dependencies: ["Carousel adaptation"],
          blocker: "",
          hook: "The agencies shipping fastest are not creating more. They are extracting more value from every shoot day.",
          cta: "Comment repurpose if you want the full process map.",
          campaignId: "campaign-3",
          sponsorId: "",
          lastOpportunityScore: 79,
          thumbnailPlan: { direction: "Authority",
          textPlacement: "Top banner",
          clutter: "Balanced",
          focus: "Result reveal" } },
        { id: "content-5",
          title: "Carousel Series: Our Weekly Creator Ops Meeting Template",
          description: "Process-driven carousel for teams that need better weekly planning and fewer reactive check-ins.",
          platform: "Carousel",
          stage: "Review",
          ...
          dependencies: [],
          blocker: "",
          hook: "One 20-minute ops meeting can eliminate a week of chaotic follow-up if the agenda is built correctly.",
          cta: "Save the carousel and run this agenda with your team this week.",
          campaignId: "campaign-1",
          sponsorId: "",
          lastOpportunityScore: 72,
          thumbnailPlan: { direction: "Relief",
          textPlacement: "Bottom tag",
          clutter: "Balanced",
          focus: "Big claim" } },
        { id: "content-6",
          title: "Shorts Lesson: The Thumbnail Mistake That Makes Good Videos Look Weak",
          description: "Packaging lesson for creators improving click-through rate without over-designing cover art.",
          platform: "Shorts",
          stage: "Published",
          deadline: todayOffset(-2),
          priority: "High",
          assigneeId: "team-4",
          topic: "thumbnail clarity",
          dependencies: [],
          blocker: "",
          hook: "If the viewer cannot decode the story in one glance, your packaging is already making the video work too hard.",
          cta: "Share this with the editor who keeps over-designing your covers.",
          campaignId: "campaign-2",
          sponsorId: "",
          lastOpportunityScore: 84,
          thumbnailPlan: { direction: "Curious",
          textPlacement: "Minimal text",
          clutter: "Minimal",
          focus: "Result reveal" } }
      ],
      ...
      scripts: [
        { id: "script-1",
          title: "Creator Ops Launch Video",
          contentId: "content-1",
          sections: { hook: "Most creator teams are not losing consistency because of bad ideas. They are losing because the operation behind the content is broken.",
          intro: "Today I am showing the exact creator ops system that pulled our weekly publishing rhythm out of chaos.",
          story: "We reached a point where every upload was late, sponsor tasks stacked up, and editing turned into recovery instead of process.",
          value: "I will break down planning, packaging, approvals, repurposing, and the weekly review cadence that fixed the bottlenecks.",
          proof: "After implementing this structure, our team recovered consistency, protected sponsor delivery, and reduced review churn.",
          cta: "Download the checklist and run the same weekly operating rhythm with your own team." } },
        { id: "script-2",
          title: "Sponsor Delay Breakdown",
          contentId: "content-3",
          sections: { hook: "A slow sponsor approval does not just delay one reel. It can quietly damage the entire week behind it.",
          intro: "Sponsor chaos usually looks like a communication problem, but it is really an operations design problem.",
          story: "We watched one delayed legal change push editing, packaging, and short-form cutdowns into overtime.",
          value: "Here is how to structure handoffs, approval checkpoints, and sponsor locks before production breaks.",
          proof: "The last three sponsor-backed launch waves using this structure cleared faster and needed fewer late changes.",
          cta: "Use this workflow the next time a sponsor asks for a late revision." } }
      ],
      campaigns: [
        { id: "campaign-1",
          name: "Creator Ops Launch Week",
          offer: "Content Operations Audit",
          waveLabel: "Wave A",
          deadline: todayOffset(6),
          progress: 64,
          sequence: ["Hero YouTube publish",
          "Instagram reel cutdown",
          "Carousel meeting template",
          "Newsletter follow-up"] },
        { id: "campaign-2",
          name: "TikTok Sprint for Pod Studios",
          offer: "Short-Form Repurposing Pack",
          waveLabel: "Wave B",
          deadline: todayOffset(5),
          progress: 46,
          sequence: ["Problem clip",
          "Retention lesson",
          "CTA follow-up short",
          "Email teaser"] },
        { id: "campaign-3",
          name: "Agency Distribution Proof Series",
          offer: "Agency Content System Workshop",
          waveLabel: "Wave C",
          deadline: todayOffset(10),
          progress: 72,
          sequence: ["LinkedIn proof post",
          "Carousel adaptation",
          "Client shoot case story"] }
      ],
      sponsors: [
        { id: "sponsor-1",
          name: "StudioStack",
          ...
          deadline: todayOffset(2),
          deliverables: ["Integrated mention in hero YouTube video",
          "LinkedIn support post",
          "Usage rights note"],
          notes: "Brand wants emphasis on multi-editor coordination and production visibility." },
        { id: "sponsor-2",
          name: "CueTrack",
          value: 8500,
          approval: "Briefing",
          ...
          deadline: todayOffset(1),
          deliverables: ["Integrated mention in Instagram reel series",
          "Short-form adaptation approval"],
          notes: "Legal review is the current blocker and must clear before scheduling." }
      ],
      approvals: [
  ...
  function upsertById(list, item) {
    let index = list.findIndex(function (entry) { return entry.id === item.id; });
  ...
  function dateDiffDays(value) {
    ...
    let now = new Date(current.getFullYear(), current.getMonth(), current.getDate());
    let target = new Date(value + "T00:00:00");
  ...
  function getMemberName(id) {
    let member = findById(state.team, id);
